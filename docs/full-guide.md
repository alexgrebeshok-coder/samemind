<!-- Archived full homepage (pre readme-honest-pack split, 2026-07-21).
     Short landing: ../README.md — deep topics also in sibling docs/*.md. -->
...
| Wire format | ad hoc, tool-specific | [Google OKF v0.1](interop.md) — `export`/`import` speak it directly |
| Identity | none — memory is undifferentiated notes | `Identity`/`User`/`EngineRule` concepts, compressed into a budget-bounded `brief` |
| Work tracking | none, or a separate tool | append-only [event ledger](event-ledger.md) + [kanban board](#board) in the same bundle, no glue |
...
[docs/adapters.md](adapters.md) for the full matrix.
...
[docs/memory-protocol.md](memory-protocol.md). Paste-ready rules:
...
[docs/compaction-recipe.md](compaction-recipe.md).
...
deleting history: see [docs/memory-hygiene.md](memory-hygiene.md).
...
[`docs/identity-layer.md`](identity-layer.md)) hold voice/values/boundaries,
...
[`docs/work-discipline.md`](work-discipline.md)) plus the knowledge-cycle
`💡 Ideas` section (see "The knowledge flywheel" below) and a **🔥 Open failures**
section fed by the [event ledger](event-ledger.md): what's queued, what's
...
`Analysis`, `Research`, `Idea` (see [`docs/knowledge-cycle.md`](knowledge-cycle.md))
...
protocol: [`docs/memory-protocol.md`](memory-protocol.md)). `samemind
...
| `memory_ledger_append` | `{topic, phase, status?, action, artifact?, ref?}` → append one event to `ledger/events.jsonl`. `actor` comes from `SAMEMIND_AGENT` (default `mcp`) — same contract as `memory_write_inbox`. See [docs/event-ledger.md](event-ledger.md). |
...
- **Google OKF v0.1** wire shape — `samemind export` / `import` exchange shareable packs (`okf_version: "0.1"`); `--to-gbrain` maps concepts to [garrytan/gbrain](https://github.com/garrytan/gbrain) pages — see [docs/interop.md](interop.md)
...
`samemind capture --engine <id>` (see [docs/session-capture.md](session-capture.md)):
...
Methodology and current hit@1 / hit@3 numbers: [docs/benchmark.md](benchmark.md).
