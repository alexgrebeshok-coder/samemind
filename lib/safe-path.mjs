...
export function assertSafeConceptId(id, baseDir) {
  ...
  if (!raw) throw Object.assign(new Error('id: empty'), { code: 'INVALID_ID' });
  const rel = raw.replace(/^\/+/, '').replace(/\.md$/, '');
  if (!rel) throw Object.assign(new Error(`id: empty after normalization "${id}"`), { code: 'INVALID_ID' });
  if (rel.split('/').some(seg => seg === '' || seg === '..' || seg === '.')) {
    throw Object.assign(new Error(`id: unsafe path "${id}" (.. / . / empty segments not allowed)`), { code: 'INVALID_ID' });
  }
  const target = resolve(baseDir, `${rel}.md`);
  const root = resolve(baseDir) + sep;
  if (!target.startsWith(root)) {
    throw Object.assign(new Error(`id: path traversal "${id}" -> ${target}`), { code: 'INVALID_ID' });
  }
  return rel;
}
